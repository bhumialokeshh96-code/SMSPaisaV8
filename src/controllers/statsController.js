...
const getOverview = async (req, res) => {
  try {
    const [wallet, totalSmsSent, totalDelivered, activeDevices] = await Promise.all([
      ...
      // Yahan SENT add kiya gaya
      prisma.smsLog.count({ where: { userId: req.user.id, status: { in: ['SENT', 'DELIVERED'] } } }),
...
const getDailyStats = async (req, res) => {
  try {
    ...
    const [sent, delivered, failed, earningsResult] = await Promise.all([
      ...
      // Yahan SENT add kiya gaya
      prisma.smsLog.count({ where: { userId: req.user.id, status: { in: ['SENT', 'DELIVERED'] }, createdAt: { gte: startOfDay, lte: endOfDay } } }),
  ...
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const [sent, delivered, failed, earningsResult] = await Promise.all([
        ...
        // Yahan SENT add kiya gaya
        prisma.smsLog.count({ where: { userId: req.user.id, status: { in: ['SENT', 'DELIVERED'] }, createdAt: { gte: dayStart, lte: dayEnd } } }),
  ...
  try {
    ...
    while (current <= endOfMonth) {
      ...
      const [sent, delivered, failed, earningsResult] = await Promise.all([
        ...
        // Yahan SENT add kiya gaya
        prisma.smsLog.count({ where: { userId: req.user.id, status: { in: ['SENT', 'DELIVERED'] }, createdAt: { gte: weekStart, lte: weekEnd } } }),
